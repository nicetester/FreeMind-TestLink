...
class FreeMind(object):
    ...
    def _gen_req_xml(self, item_list, doc_title, filename, prefix):
    ...
    def link_pfs2tds(self, tds_url, tc_url, pfs_url):
        ...
        res = self._build_fm_traceability(tds_tc_file, pfs_file, tc_req_list, self.tds_url.replace('.mm', '[TDS-TC-PFS].mm'))
    ...
    def _read_tc_from_xml(self, xml_file, fm_file, tc_req_list):
        ...
        prefix_list.extend(self.pfs_prefix.split(','))    #Could be multiple PFS prefix since some requirements will be reused between projects.
    ...
    def _add_tc_details(self, tc_root, fm_root):        
        for child in tc_root:            
            if child.tag == 'testsuite':
                ...
                testsuite_node = ET.SubElement(fm_root,  'node', {'COLOR' : '#990000', 'FOLDED':"true", 'TEXT': child.attrib['name']})  
...
#        return res
                        ...
                        if (child.attrib['TEXT'].split(PREFIX_TITLE_SEP)[0] <> child.attrib['TEXT']) :
                            if child.attrib['TEXT'].split(PREFIX_TITLE_SEP)[0] <> prefix[4:]:                                                                
                                self.logger.error(self.log_prefix + \
                                    ...
                                    (child.attrib['TEXT'].split(PREFIX_TITLE_SEP)[0], child.attrib['TEXT'] , prefix[4:]))   
                                return None
    ...
    def extract_requirements(self, excel_file):
        pmr_list = []
        pfs_list = []
        pfs_pmr_list = []  
        pmr_pfs_list = []
        ...
        if not os.path.exists(excel_file):
            self.logger.error(self.log_prefix + \
                ...
                (excel_file))             
            return None
        res = self._read_req_from_xls(excel_file, pmr_list, pfs_list, pfs_pmr_list)
        ...
        res = self._gen_req_xml(pfs_list, title, self.pfs_url, self.pfs_prefix)  
        
        #TODO: Need to get the pmr_list, pfs_list, pfs_pmr_list from xml once the TestLink supports exporting relations between requirements.
        ...
        res = self._build_fm_traceability(self.pfs_url.replace('.xml', '.mm'), self.pmr_url.replace('.xml', '.mm'), pfs_pmr_list , self.pfs_url.replace('.xml', '[PFS-PMR].mm'))
        res = self._reverse_links(pfs_pmr_list, pmr_pfs_list)
        res = self._build_fm_traceability(self.pmr_url.replace('.xml', '.mm'), self.pfs_url.replace('.xml', '.mm'), pmr_pfs_list, self.pmr_url.replace('.xml', '[PMR-PFS].mm'))
        return res
        ...
        for dst_node in dst_fm_root.iter('node'):
            ...
            if dst_node.attrib.has_key('LINK') and (dst_node.attrib['TEXT'] not in new_added_nodes): 
                ...
                for req_link_id in req_links:
                    ...
                    link_found = False
                    for src_node in src_fm_root.iter('node'):
                        if src_node.attrib.has_key('LINK') and (src_node.attrib['TEXT'].split(PREFIX_TITLE_SEP)[0] == req_link_id):
                            link_found = True
                            dst_node.append(src_node)
                            new_added_nodes.append(src_node.attrib['TEXT'])
                            self.logger.info(self.log_prefix + \
    ...
    def _gen_req_freemind(self, req_list, title, output_file, prefix):
        ...
        for group in req_list:
            ...
            for i, req_item in enumerate(group[1]):
                ...
                node_link = self.testlink_url + '/linkto.php?tprojectPrefix='+ self.repo_prefix + '&item=req&id=' + prefix + req_item[REQ_ID]
    ...
    def _read_req_from_xls(self, file_name, pmr_list, pfs_list , trace_list):
